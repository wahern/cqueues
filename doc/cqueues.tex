...
There is no separate \texttt{./configure} step. System introspection occurs during compile-time. However, the ``configure'' make target can be used to cache the build environment.
...
\item[\texttt{lua51path}] install path for Lua 5.1 modules, e.g. \texttt{\$(prefix)/share/lua/5.1}
\item[\texttt{lua51cpath}] install path for Lua 5.1 C modules, e.g. \texttt{\$(prefix)/lib/lua/5.1}
\item[\texttt{lua52path}] install path for Lua 5.2 modules, e.g. \texttt{\$(prefix)/share/lua/5.2}
\item[\texttt{lua52cpath}] install path for Lua 5.2 C modules, e.g. \texttt{\$(prefix)/lib/lua/5.2}

\end{description}

All the common GNU-style compiler variables are supported, including \texttt{CC}, \texttt{CPPFLAGS}, \texttt{CFLAGS}, \texttt{LDFLAGS}, and \texttt{SOFLAGS}. Note that you can specify the path to both Lua 5.1 and Lua 5.2 include headers at the same time in CPPFLAGS; the build system will work things out to ensure the correct headers are loaded at compile-time.
...
\subsubsection[\fn{socket.setmode}]{\fn{socket.setmode([input] [, output])}}
	Set the default I/O modes. See \fn{socket:setmode}.

\subsubsection[\fn{socket.setbufsiz}]{\fn{socket.setbufsiz([input] [, output])}}
	Set the default I/O buffer sizes. See \fn{socket:setbufsiz}.

\subsubsection[\fn{socket.setmaxline}]{\fn{socket.setmaxline([input] [, output])}}
	Set the default I/O line-buffering limits. See \fn{socket:setmaxline}.
...
\subsubsection[\fn{socket:setmode}]{\fn{socket:setmode([input] [, output])}}
See \fn{socket.connect} for mode flags. Either mode can be nil or none, in which case the mode is left unchanged.
...
Note that for SOCK\_DGRAM sockets, each message is treated as-if EOF was reached. The slurp operation returns a single datagram, and line-buffered operations will return the remaining text in a message even without a terminating \lf. Datagrams will be truncated if the message is larger than the input buffer size.
...
OpenSSL will automatically attempt to seed itself from the system. The only time this could theoretically fail is if /dev/random (or similar) were not visible or could not be opened. This might happen if within a chroot jail, or if a file descriptor limit were reached.
