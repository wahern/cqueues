...
Returns true on success, false and an error if the name is too long or if the platform does not support this function.

\subsubsection[\fn{thread:getname}]{\fn{thread:getname(name)}}
Get the name of the thread. Supported on Linux, NetBSD \& Mac OS X.

Returns the name string, or nil and an error if the platform does not support this function.
