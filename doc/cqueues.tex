...
\subsubsection[\fn{socket:starttls}]{\fn{socket:starttls([context])}}
Place socket into TLS mode, optionally using the \module{openssl.ssl.context} object as the configuration prototype. Returns immediately.
...
Returns the X.509 ``basic constraint'' flags. If specified, `which' should be one of ``CA'' or ``pathLen'', which returns that flag as a boolean. If none are specified, returns a table with boolean fields ``CA'' and ``pathLen''.

\subsubsection[\fn{x509:setBasicConstraints}]{\fn{x509:setBasicConstraints\{ $\ldots$ \}}}

Sets the basic constraint flag according to the defined boolean field values for ``CA'' and ``pathLen''.
...
\subsubsection[\fn{context.new}]{\fn{context.new()}}
